...
namespace GoogleRezension\Extensions;
defined('ABSPATH') or die();


use Exception;
use Goggle\Rezension\Trait_Extension_Defaults;
use stdClass;
use Throwable;
...
final class WWDH_Api_Ajax
{
     ...
     * @return static
     */
    public static function instance(string $version, string $basename, Google_Rezensionen_Api $main): self
    {
    ...
    public function wwdh_api_ajax_handle(): stdClass
    {
        ...
        switch ($this->method) {
            case'get_public_api_commands_select':
                ...
                $path = GOOGLE_REZENSIONEN_API_ADMIN_DIR . 'js' . DIRECTORY_SEPARATOR . 'templates';
                ...
                switch ($command) {
                    case '1':
                        ...
                        if (isset($resource->data) && $resource->data) {
                            $idRsaDir = GOOGLE_REZENSION_ID_RSA_DIR . 'public_id_rsa';
            ...
            case 'load_extension_preview_data':
                ...
                foreach ($extension as $tmp) {
                    ...
                    $ret_item = [
                        ...
                        'data' => [
                            ...
                            'extension_logo' => GOGGLE_REZENSION_EXTENSION_PREVIEW_URL . $tmp . '/' . $data->extension_logo
                        ]
                    ];
                    $retArr[] = $ret_item;
                }
                ...
                $twig = new Environment($twig_loader);
                try {
                    $template = $twig->render($template . '.twig', ['data' => $retArr]);
                    $responseJson->template = preg_replace(array('/<!--(.*)-->/Uis', "/[[:blank:]]+/"), array('', ' '), str_replace(array("\n", "\r", "\t"), '', $template));
            ...
            case 'load_license_data':
                ...
                $data = json_decode(file_get_contents($dataUrl));
                if(!$data || !isset($data->extension_logo) && !$data->extension_logo){
                    return $responseJson;
                }
            ...
            case'help_extension':
                ...
                $fileData = $extensionOptions->get_wwdh_extension_description_template($extension, $template . '.twig');
                if (!$fileData->status) {
                    $responseJson->msg = 'Help not found!';
                    return $responseJson;
                }
                ...
                $hlpExDir = dirname($fileData->path) . DIRECTORY_SEPARATOR . 'help-example' . DIRECTORY_SEPARATOR;
                $dirList = apply_filters($this->basename.'/get_directoryL_list',$hlpExDir);
                ...
                if (is_dir($hlpExDir)) {
                    foreach ($dirList as $file){
                        if(is_file($hlpExDir . $file)){
                            $lines = file($hlpExDir . $file);
                            $html = '<pre class="pre-templates">';
                            foreach ($lines as $line) {
                                $html .= '<span class="number">' .htmlspecialchars($line) . '</span>';
                            }
                            $html .= '</pre>';
                            $helpPre[] = $html;
                        }
                ...
                $data = [];
                $data['methods'] = ['Fetch Example', 'jQuery Example', 'Javascript Example', 'PHP Example'];
                $data['scripts'] = $helpPre;
            ...
            case'activate_extension':
                ...
                try {
                    ...
                    $responseJson->msg = '<b class="strong-font-weight">' . __('Server Response', 'google-rezensionen-api') . ':</b> ' . $e->getMessage();
                    return $responseJson;
                }

                if (!$licenseData->status) {
                    $responseJson->msg = '<b class="strong-font-weight"> ' . __('Server Response', 'google-rezensionen-api') . ':</b> ' . $licenseData->msg;
                    return $responseJson;
                }
    ...
    public function get_extension_license_data($licenseData)
    {
        ...
        if ($licenseData->activated_time) {
            ...
            $licenseData->activatedTime = sprintf($timeStart[0] . ' %s ' . $timeStart[1] . ' %s', __('at', 'google-rezensionen-api'), __('Clock', 'google-rezensionen-api'));
        }

        if ($licenseData->time_limit_aktiv) {
            $licenseData->time_limit > 1 ? $licenseData->limitMonat = __('Months', 'google-rezensionen-api') : $licenseData->limitMonat = __('Month', 'google-rezensionen-api');

            $timeEnd = explode(' ', $licenseData->licenseEndTime);
            $licenseData->licenseEndTime = sprintf($timeEnd[0] . ' %s ' . $timeEnd[1] . ' %s', __('at', 'google-rezensionen-api'), __('Clock', 'google-rezensionen-api'));
        } else {
            $licenseData->licenseEndTime = __('unlimited', 'google-rezensionen-api');
            $licenseData->limitMonat = __('unlimited', 'google-rezensionen-api');
        }

        if (!$licenseData->url_limit_aktiv) {
            $licenseData->url_limit = __('unlimited', 'google-rezensionen-api');
        }

        if ($licenseData->url_limit_aktiv) {
            $licenseData->url_limit > 1 ? $licenseData->urlLimitTxt = __('Installations', 'google-rezensionen-api') : $licenseData->urlLimitTxt = __('Installation', 'google-rezensionen-api');
        } else {
            $licenseData->url_limit = __('unlimited', 'google-rezensionen-api');
