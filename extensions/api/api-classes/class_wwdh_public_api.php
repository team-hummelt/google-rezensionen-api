...
class Wwdh_Public_Api
{
    ...
    public function __construct(string $version, string $basename, Google_Rezensionen_Api $main)
    {
        ...
        if (!get_option($this->basename . '-api-options')) {
            $plugin_extension_helper->wwdh_set_default_options();
        }
    ...
    public function wwdh_get_public_resource_method($command, $api_url = '', $addBody = []): object
    {
        ...
        if (!get_option('extension_access_token')) {
            try {
                ...
                do_action($this->basename.'/set_api_log','error', $e->getMessage());
            }
        }
    ...
    public function wwdh_public_api_select_commands($get_id = 0): object
    {
        ...
        if ($get_id) {
            foreach ($commands as $tmp) {
                if ($tmp['id'] == $get_id) {
                    return $plugin_extension_helper->ERArrayToObject($tmp);
                }
            }
        }

        return $plugin_extension_helper->ERArrayToObject($commands);
    }
