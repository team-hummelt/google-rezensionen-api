 ...
 */
class GoogleRezensionExtensions
{
     ...
     */
    public function __construct(string $basename, string $version, Google_Rezensionen_Api $main, Environment $twig)
    {
    ...
    public function wwdh_check_extension_preview_updates()
    {
        ...
        foreach ($extDirs as $tmp) {
            ...
            $checkUpdate = $googleRezensionPublicApi->wwdh_get_public_resource_method('update_preview', '', $body);

            if ($checkUpdate->status) {
                if ($checkUpdate->update) {
                    $plugin_extension_helper->wwdhDestroyDir($this->main->get_extension_preview() . $tmp['filename']);
                    $url = get_option($this->basename . '-api-options')['public_api_preview_url'] . '/update/' . $tmp['filename'];
                    $zipFile = $googleRezensionPublicApi->wwdh_api_public_download($url, $body);
        ...
        try {
            $getExtension = $googleRezensionPublicApi->wwdh_get_public_resource_method('get_extensions');
        ...
        if (isset($getExtension->data) && $getExtension->data) {
            foreach ($getExtension->data as $tmp) {
                ...
                try {
                    $zipFile = $googleRezensionPublicApi->wwdh_api_public_download($tmp->download_url, $body);
        ...
        foreach (scandir($this->main->get_extension_preview()) as $file) {
            ...
            if (is_dir($this->main->get_extension_preview() . $file)) {
                if (!in_array($file, $apiExtFolder)) {
                    $plugin_extension_helper->wwdhDestroyDir($this->main->get_extension_preview() . $file);
                }
            }
        }
    }
    ...
    public function wwdh_get_extension_preview_url_data($folder): string
    {
        ...
        if (is_dir($dir . $folder)) {
            return GOGGLE_REZENSION_EXTENSION_PREVIEW_URL . $folder . '/extension.json';
        }
        return '';
    }
    ...
    public function get_extension_previews_lang_url($extension): string
    {
        ...
        if (is_file($folder . $path . $curLang . '.json')) {
            return GOGGLE_REZENSION_EXTENSION_PREVIEW_URL . $extension . '/language/json/' . $curLang . '.json';
        } else {
            return GOGGLE_REZENSION_EXTENSION_PREVIEW_URL . $extension . '/language/json/en.json';
        }
    }
    ...
    public function get_wwdh_extension_description_template($extension, $template_name): object
    {
        ...
        $description_url = GOGGLE_REZENSION_EXTENSION_PREVIEW_URL . $extension . '/' . 'includes/description/';
        if (is_file($file)) {
            $return->status = true;
    ...
    public function wwdh_check_extensions_installs($extension = ''): object
    {
        ...
        $folderExt = $this->read_wwdh_folder(GOOGLE_REZENSION_EXTENSION_DIR . 'installed' . DIRECTORY_SEPARATOR);

        $errDb = 0;
        if ($folderExt) {
            foreach ($folderExt as $folder) {
                $args = sprintf('WHERE folder="%s"', $folder);
                $dbInstalls = apply_filters($this->basename.'_get_extension', $args);
                if (!$dbInstalls->status) {
                    $dir = GOOGLE_REZENSION_EXTENSION_DIR . 'installed' . DIRECTORY_SEPARATOR . $folder;
                    if (is_dir($dir)) {
                ...
                try {
                    ...
                    if (isset($apiData->remove) && $apiData->remove) {
                        ...
                        if ($apiData->extension) {
                            $dir = GOOGLE_REZENSION_EXTENSION_DIR . 'installed' . DIRECTORY_SEPARATOR . $apiData->extension;
                            if (is_dir($dir)) {
                               apply_filters($this->basename . '/destroy_dir', $dir);
                            }
                        }
    ...
    public function wwdh_check_api_extension()
    {
        ...
        if ($dbData->status) {
            foreach ($dbData->record as $tmp) {
                ...
                $dir = GOOGLE_REZENSION_EXTENSION_DIR . 'installed' . DIRECTORY_SEPARATOR . $tmp->folder;
                if (!$apiCheck->status) {
                    if ($tmp->errors > 4) {
                        if (is_dir($dir)) {
                            apply_filters($this->basename . '/destroy_dir', $dir);
